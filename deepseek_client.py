...
used to enrich the raw DataForSEO data with:
...
The relevance and extraction-readiness scores are the model's own judgment of
...
never hardcoded. Uses DeepSeek's JSON Output mode (response_format:
{"type": "json_object"}) so the response is guaranteed valid JSON.
"""
...
CHAT_COMPLETIONS_ENDPOINT = f"{DEEPSEEK_BASE_URL}/chat/completions"

# deepseek-v4-flash is DeepSeek's fast, non-thinking model - the right fit
# for structured JSON generation like this (cheaper and quicker than the
# reasoning-heavy deepseek-v4-pro, which isn't needed here). Note: the old
# "deepseek-chat" model alias is deprecated 2026-07-24, so we target the
# real model name directly.
...
class DeepSeekError(Exception):
    """Raised when the DeepSeek API is misconfigured or returns an unusable response."""
...
def _get_api_key() -> str:
    ...
    if not api_key:
        raise DeepSeekError(
            "Missing DEEPSEEK_API_KEY. Add it to your .env file (see .env.example)."
        )
    return api_key
...
def generate_keyword_insights(seed_keyword: str, serp_data: dict) -> dict:
    """
    Calls DeepSeek with the seed keyword + SERP context and returns a dict:
      {
    ...
    try:
        response = requests.post(
            CHAT_COMPLETIONS_ENDPOINT, json=payload, headers=headers, timeout=60
        )
    ...
    try:
        ...
        raise DeepSeekError(
            f"Could not parse DeepSeek response as JSON: {exc}\nRaw response: {raw_text[:500]}"
        ) from exc
