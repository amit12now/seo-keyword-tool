...
2. DataForSEO returns top organic results, People Also Ask, related keywords,
   ...
   monthly trend).
3. DeepSeek turns that raw data into NLP keyword ideas and fan-out query ideas,
...
4. Real DataForSEO search volume is shown only for the primary keyword and
   ...
   Also Ask show DeepSeek's relevance score instead.
5. An "Answer Engine Optimization" (AEO) section shows a format-matched
   ...
   score, plus a concise answer for every People Also Ask question. Insights
   are generated by DeepSeek.
...
from deepseek_client import generate_keyword_insights, DeepSeekError
...
# them into os.environ so dataforseo_client.py / deepseek_client.py - which
# only know about os.getenv() - work unchanged in both environments.
for _key in ("DATAFORSEO_LOGIN", "DATAFORSEO_PASSWORD", "DEEPSEEK_API_KEY", "DEEPSEEK_MODEL"):
...
st.markdown(
    ...
    '<div class="topbar-sub">Powered by DataForSEO + DeepSeek</div></div>'
    '</div></div>',
...
with st.spinner("Generating keyword insights with DeepSeek..."):
    try:
...
# Align PAA relevance + AEO answers with the PAA list by position (DeepSeek is
...
# (related) keywords - NLP keywords, fan-out queries, and PAA use DeepSeek's
...
st.markdown(
    '<div class="aeo-note">For your reference: these are DeepSeek\'s best estimate of how an '
...
st.markdown(
    '<div class="footer-note">Built with Streamlit &middot; Data provided by DataForSEO &middot; Insights by DeepSeek</div>',
    unsafe_allow_html=True,
)
